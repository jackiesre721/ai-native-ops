# 第11章 灰度发布与生产变更风险治理
<!-- 第三篇 声明式交付体系 ｜ 常规章（极简流量·单案例·严控边界） ｜ 状态：正文写作中 -->
...
Argo Rollouts 金丝雀的最小实操：

1. **用 Rollout 替代 Deployment**：Rollout 是 Argo Rollouts 的发布负载，支持金丝雀。
2. **定义金丝雀步骤**：流量比例阶梯（如 5%→25%→50%→100%），每步停留观测。
3. **挂分析**：每步挂钩 SLO 分析，异常自动回退。
4. **流量切分**：配流量实现（11.5 节单案例）。
