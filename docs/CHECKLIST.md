...
- [ ] 新业务 = 提交目录即接入（ApplicationSet，9.4 ③），平台零介入？
...
**15.3 KEDA分层弹性体系（与第6章HPA严格解耦）：聚焦业务层指标（队列积压、请求速率、在线连接数），解答「如何通过平台能力补齐原生弹性短板，构建业务级自治弹性」**
...
**15.5 AIOps落地路径：七步施工路线（数据供给线→口径分级→自愈通道→建议式引擎→评测门禁→白名单自动执行→L3）**

- [ ] 步 0 数据供给线四件套 60s 可装配（12/12.4/9.6 已通）？
