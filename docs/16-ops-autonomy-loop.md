...
> 某天，reco-llm 的排队指标（16.1② 查的 `vllm:num_requests_waiting`）因一个埋点 bug 恒为 10000——真实排队接近 0。它的 ScaledObject 恰好没配 `maxReplicaCount`（KEDA 默认放行到 100，对 GPU 负载等于没有上限）。**先猜：一小时后会发生什么？**

揭晓：KEDA 忠实按"排队 10000"折算期望副本 → 副本扩到节点池装不下 → ACK 节点池自动伸缩开始补节点 → GPU 节点（gn7i）一台一台起，每台都是钱。一小时后云账单烧掉平时**一周**的 GPU 费——而真正的故障（埋点 bug）与扩容毫无因果关系：负载根本不忙。KEDA 没做错任何事，它只是**放大了错误信号的购买力**：信号错一个量级，资源消耗就跟着错一个量级。结论：护栏（max 上限/扩容次数预算/信号异常检测）不是可选项，是自动化的**准入条件**——护栏不齐的 ScaledObject 不允许进生产。
...
第四行是全表要害：L2 的"智能"止步于**响应信号**，信号真伪只能靠护栏（封顶/预算/异常检测）与校验（SLO 回归）对冲——这正是 16.3 思想实验用一周 GPU 费买来的教训。
...
三数字体感：自动处置占比 ≥60% = 值班每 10 次处置 6 次不用醒；误处置率 ≤5% = 20 次里最多错 1 次，且错的 1 次还有 10 min 校验窗兜底；人工升级率 20%–40% = 每 5 次自动处置约 1–2 次转人工；同源的 L3 护栏"扩容 ≤2 次/日" = 机器每天最多两次自作主张加 GPU，其余都要人点头（18.9）。
...
本节是 L2 运维自治的**完整闭环定义**：**感知→预判→处置→校验→复盘，全闭环的 L2 实现了"业务稳定性自动化治理"**——这正是全书第二层自治的目标。L2 闭环的成熟度，决定了"值班被叫醒次数"能降多少。同时，L2 闭环（尤其校验与复盘）为 L3 智能自治（第 18 章）提供了可信基础与方法论模板——L3 的"观测→推理→决策→执行→校验→反馈"正是 L2 闭环在 AI 负载场景的进化版。这套闭环同时给出**运维 Agent 的 L2 形态**：处置动作映射表 = Agent 的规则库、白名单 = 权限边界、处置契约三字段 = 动作可审计、闭环三数字 = 治理 KPI——执行者无论是脚本、平台还是 Agent（自建或 STAROps 类托管产品），四件制品一件不能少（运维 Agent 能力与书内制品的完整对应见 18.9）。
...
> **本章知识收拢（带走三件事）**：
> 1. **自动化放大错误信号，护栏是准入条件**：自治系统不判断信号真伪——一个排队指标恒为 10000 的埋点 bug，能让没配 maxReplicaCount 的 KEDA 一小时烧掉一周 GPU 费。max 封顶/扩容次数预算/异常检测配齐之前，自动化没有上线资格。
