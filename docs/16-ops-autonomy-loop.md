...
云服务映射：处置通道全部落在云生态制品上——扩缩 = KEDA（16.3）+ ACK 节点池弹性（对照 EKS 托管节点组/Karpenter）；回退 = ACK 上 ArgoCD/Rollouts；P0 电话升级 = 云语音通道（13.1：阿里云语音服务/钉钉电话，对照 AWS SNS）。数字基线：校验窗 5–10 min、扩容 ≤2×、升级线 10 min——与 13.3 的 P0 30 min 止损目标衔接（自动处置 10 min 无效即转人工，仍留 20 min 人工窗口）。体感：10 min 升级线 = 两轮 5 min 校验窗——给自动处置两次自证机会，之后的每一分钟都留给人工。
