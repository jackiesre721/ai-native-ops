# 第8章【L1 机械自治】一切即代码：声明式治理全域架构
<!-- 第三篇 声明式交付体系 ｜ 知识型工程章节 ｜ 状态：终审中 -->

> 🧭 **本篇路标 · 第三篇「全维度声明式交付体系」（第 8–10 章）**：本篇把 L1 机械自治从运行时延伸到交付面——Desired State 进 Git、系统持续收敛、变更安全迁移。Agent 的智能制品（分诊器 prompt、评测集、规则表、白名单）走同一条声明式供应链与 GitOps 变更纪律（15.5②）——智能不豁免治理；变更事实供给线也由此打通，根因分析不丢「第一嫌疑」。没有交付层声明式，L3 的制品不可审计、变更不可回放。

> 本章定位：第三篇开篇——建立「声明式系统」的完整心智模型：Drift 为什么必然发生（8.1）、声明式的本质是什么（8.2）、Git 为什么能当真相源（8.3），再到两大工程载体——Terraform 管集群之下（8.4）、Helm 管集群之上（8.5），最后合成全域知识地图与双层 Drift 治理（8.6）。分层边界一句话：**集群之下 Terraform、集群之上 GitOps**——为什么这样分是 8.4 要推导的核心知识，不是规定。
> **主线定位**：本章定义 Desired State——L1/L2 全部控制循环的期望状态从此而来；分诊器 prompt 与评测集同样进 Git、走 PR（15.5②），与基础设施共用一条声明式供应链。**承上启下**：承第 5 章声明式思维（底座篇既毕，把 Desired State 写成代码）；启第 9 章 GitOps（定义 Desired State → 持续收敛）。
