...
> 本章定位：讲 K8s 作为云原生统一操作系统与生产负载统一运行底座的架构与生产治理。**全书生态锁定托管 K8s（阿里云 ACK 主参考、AWS EKS 对照）**——控制面由云厂商 SLA 保证（4.2 一图划界），笔墨在节点面、云集成（RRSA/IRSA、CCM、CSI）、版本治理与原生短板。
> **主线定位**：本章为托管底座是 L1 机械自治的地基——控制器与调谐闭环运行其上（三层自治见 1.5；L3 = 运维 Agent 引擎，15.4⑤/15.5）。 **主旨绑定**：业务负载与运维 Agent 的统一运行底座——AIOps 不是绕开 K8s 另起系统，而是长在托管底座上（3.1 主从：底座为主旨服务）。 **承上启下**：承第 3 章范式定纲（进入 How：底座施工；制品由第 2 章供给）；启第 5 章调谐闭环（控制器跑在底座上）。
