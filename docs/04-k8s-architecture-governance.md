...
> 本章定位：讲 K8s 作为云原生统一操作系统与 AI 负载统一运行底座的架构与生产治理。**全书生态锁定托管 K8s（阿里云 ACK 主参考、AWS EKS 对照），自建 K8s 一律不讲**——控制面由云厂商 SLA 保证（4.2 一图划界），笔墨在节点面、云集成（RRSA/IRSA、CCM、CSI）、版本治理与原生短板。
...
- 一负载一底座：传统服务用虚拟机、容器服务用容器编排、AI 直跑裸机，底座随负载 proliferate。
- 底座间无统一抽象：调度、网络、存储、可观测各搞各的，跨底座治理不可能。
- AI 负载游离于底座之外，脱离声明式与可观测体系，回到第一代运维（第 1 章）。

失效根因：**没有把 K8s 当作统一操作系统来用**——它的核心价值不是"跑容器"，而是让所有负载（含 AI）共享同一套治理范式。
...
| **AI 负载统一运行底座** | 通过设备插件/CRD 扩展，让 AI 负载跑在同一底座上 | 需扩展机制（GPU 调度等，第 17 章） |
...
| 自建 kubeadm | **不采用** | 自管 | 控制面 SLA/升级/etcd 备份全自担，仅此一句，后文不再出现 |

**为什么不自建**：自建控制面至少 3 台 ECS（≈ ¥3000+/月）+ 每年 4 次以上小版本升级与 etcd 备份人力；托管 Pro 月费不到其两成，换回控制面 SLA 与升级托管。这个差距随集群数量线性放大。

权衡的核心：**统一底座牺牲"为每种负载特调最优"的灵活性，买回"一套治理范式覆盖所有负载"的可扩展性**。规模化下后者完胜。

### 最小可行方案

1. **选托管形态**：生产选 ACK Pro（对照 EKS），测试可用基础版；不自建。
2. **所有负载上 K8s**：传统服务容器化上 K8s，AI 负载通过设备插件上 K8s（第 17 章）。
3. **扩展而非另起**：K8s 不够用的能力（GPU 调度、业务弹性）通过 CRD/Operator/设备插件扩展，不另立底座。
...
- 集群本身的创建走声明式（Terraform `cs_kubernetes` / `aws_eks_cluster`），归第 9 章"集群之下 Terraform、集群之上 GitOps"。
- 统一交付：Helm + ArgoCD 承载所有负载（第 9、10 章）；统一可观测：VM/Loki/Tempo 采集所有负载（第 12 章）。

### 典型故障案例

某团队 AI 推理服务跑在裸机 ECS，传统服务跑在 ACK，两者调用链路跨底座断裂。一次推理服务 OOM 导致上游网关报错，因日志/链路两套体系，定位耗时 25 分钟。推理服务迁上 ACK、共享可观测后，同类故障定位压到 3 分钟（trace 直接贯通网关→推理 Pod）。
...
统一底座是三层自治的承载平台：L1 机械自治、L2 运维自治的控制循环都跑在托管 K8s 上；L3 的决策也通过 K8s 扩展机制执行。底座统一，三层自治才能统一作用于所有负载。
...
用托管 K8s 的团队，却按自建集群的提纲在学：背 etcd 多数派、apiserver 多实例——**把云厂商的活当自己的活**。而真正会翻车的地方没人管：节点池没开自动修复（坏节点靠人肉）、Pod 访问云资源还在把 AK/SK 塞进 Secret（泄露即全挂）、云负载均衡被误删没人知道。

### 传统方案失效原因

- **自建视角运维托管集群**：精通 etcd 却不会配节点池，学错了重点。
...
| 抢占式实例 | 无状态/可重算负载开启 | 折扣常见按量价 50%–90% off，须配驱逐容忍（14.3） |
| 规格 | 通用 ecs.u1 / g8i 系列；GPU gn7i（第 17 章） | 单池同规格，容量估算才线性 |
| 弹性伸缩 | 开启节点池弹性（按负载） | 与 Pod 层 HPA/KEDA 分层（7.5/16.3） |

**③ SLB 防误删三件套**（云 LB 由 CCM 按 Service 自动创建，误删=入口全断）：

```yaml
# Service 暴露入口时的最小注解（完整流量治理见第 8 章）
apiVersion: v1
kind: Service
metadata:
  name: demo-api
...
kubectl -n kube-system logs deploy/ccm --tail=50           # 看 SLB 创建/绑定失败原因（配额/权限/证书）
...
拆到底是**用自建视角运维托管集群**——可靠性边界没划清：该云厂商管的瞎操心，该自己管的（节点池/云身份/云集成）在裸奔。
...
云保证层是 L1 机械自治的地基——云厂商替你兜底控制面，你才能把自治建在节点面与工作负载上。RRSA/IRSA 同时是自治的安全边界：L2/L3 自治系统访问云资源同样走云身份临时凭据——权限可控、可审计、可撤销。
...
4. **权衡点监控**：API 延迟、调度排队纳入观测（第 12 章）。
...
| Events 关键字 | 责任组件 | 去向 |
|---|---|---|
| `insufficient cpu/mem` | 调度器（资源不足） | 7 章 requests 校准 / 节点池扩容 |
| `untolerated taint` | 调度器（污点拦截） | 7 章容忍 / 节点池标签匹配 |
| `volume node affinity conflict` | 调度器+CSI（存储拓扑） | 8 章 StorageClass 可用区 |
| `no nodes available` | 调度器（选择器空集） | 节点池 label/selector 核对 |
| 事件空白但副本不涨 | 控制器 | 下查 conditions |

**② 控制器卡住**：`kubectl get deploy <d> -o yaml | grep -A15 conditions`——`ProgressDeadlineExceeded` = 滚动卡死（探针失败/资源不足），进入 7 章探针排障。
...
某集群大规模扩容时 Pod 大量 Pending，团队以为是资源不足，加节点无效（浪费 6 台 ECS 两小时）。实际 `describe` 早就写着 `untolerated taint`：新节点池默认打了 GPU 污点，通用负载进不去。按判定表走，2 分钟定位。
...
- 控制面指标（API 延迟/调度排队/工作队列）持续观测（第 12 章）。
- 所有集群客户端（ArgoCD/监控/CI）LIST 请求强制分页。

### 自动化/自治闭环

三大组件是 L1 机械自治的执行机构：API server 接收期望状态，控制器驱动收敛，调度器分配资源（第 5 章详述闭环）。本章的判定表与指标，是 L2 运维自治"风险识别"环节对这些组件的观测输入。
...
- 节点分批：每批 ≤20%，批间看核心 SLO（13.2）无异常再继续。
- 资源级备份先行：ACK 备份中心（或 Velero，8.4）做升级前快照——托管下没有 etcd 快照可回滚，**控制面升级不可回退**，回退手段=资源级重建，所以预检比回滚重要。
...
- 版本状态纳入第 14 章技术债巡检（EOL 集群 = 最高优先级技术债）。

### 自动化/自治闭环

版本治理是底座持续可靠的保障：控制面与节点持续跟随社区，L1 机械自治才运行在受支持的、有补丁的基础上。版本欠债的集群连安全补丁都拿不到——自治体系建立在沙地上。EOL 预警可自动化：定期比对集群版本与云厂商支持矩阵，落后即开工单（L2 运维自治的"风险预判"输入之一）。
...
- **硬塞方案**：用 HPA（CPU 驱动）硬套业务弹性需求（7.5）。
...
| 短板 | 原生现状 | 适配策略 | 承接 |
|---|---|---|---|
| **业务驱动弹性** | HPA 只懂 CPU/内存 | KEDA 扩展补齐 | 第 16 章 |
| **网络强隔离** | namespace 是软边界 | NetworkPolicy + 资源配额 | 附录 A |
| **AI 算力调度** | 原生支持弱 | ACK GPU 节点池 + cGPU 共享调度 | 第 17 章 |
| **多集群舰队** | 无原生舰队管理 | 单集群治理为主，多集群归 V2 | 边界外 |
| **状态存储深度 DR** | PV/PVC 基础能力 | 云盘快照/NAS 备份到 RPO/RTO 级 | 第 8 章 |

> 云托管已替你补掉的短板不用再操心：LB 集成（CCM）、云存储接入（CSI）、Pod 直通 VPC（Terway）——自建时代这三项各要一周，托管下是开箱项。
...
3. **不够用用扩展**：业务弹性 KEDA、AI 调度 ACK GPU 能力，都走声明式扩展。
4. **超出归 V2**：多集群舰队等明确划边界，不在 V1 硬做。

### 生产落地实现

- 弹性：标准负载 HPA（7.5），业务/推理负载 KEDA（16.3）。
- 隔离：NetworkPolicy + ResourceQuota + LimitRange（附录 A）。
- AI 调度：ACK GPU 节点池 + cGPU 显存隔离（17 章），不自建 GPU 调度器。
- 多集群/深度 DR：明确归 V2，V1 只做单集群治理 + 云快照级容灾（8.4）。
...
边界识别是三层自治分层的基础：L1 用 K8s 原生（调谐闭环）；L2 用扩展（KEDA/SLO）；L3 针对 AI 短板（推理调度）。每一层都是"原生不够→扩展补齐"的体现，本章边界思维贯穿全书后续所有"用扩展补原生"的决策。
