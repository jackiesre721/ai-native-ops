...
> **主线定位**：本章为托管底座是 L1 机械自治的地基——控制器与调谐闭环运行其上（三层自治总览见 1.5，理论核心为第 5/16/18 章）。
...
**为什么不自建**：自建控制面至少 3 台 ECS（≈ ¥3000+/月）+ 每年 4 次以上小版本升级与 etcd 备份人力；托管 Pro 月费不到其两成，换回控制面 SLA 与升级托管。这个差距随集群数量线性放大。体感换算：**Pro 控制面 ¥460/月 ≈ 一名工程师一小时的成本**——云上最便宜的一份生产保险。
...
>
> 揭晓：**几十上百个**——控制台默认值与 provider 声明默认值在大量字段上不一致，逐字段核对要数天。更深一层：这一年里每次控制台应急调整（改个安全组、调个节点数）都没有记录，**没人能说清集群的"当前状态"是怎么来的**——这正是 9.1 Drift 因果链在云资源层的重演。结论：收编要趁早（新集群第一天就用 Terraform 创建），漂移黑洞随时间单调变深。
...
- 节点分批：每批 ≤20%，批间看核心 SLO（13.2）无异常再继续。体感：**≤20% = 每次只敢让五分之一的节点冒险**——10 台节点的集群每批只动 2 台，爆炸半径锁死在两成容量。
