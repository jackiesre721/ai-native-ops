...
> **主线定位**：本章为托管底座是 L1 机械自治的地基——控制器与调谐闭环运行其上（三层自治见 1.5；L3 = 运维 Agent 引擎，15.4⑤/15.5）。 **主旨绑定（V1.4）**：业务负载与运维 Agent 的统一运行底座——AIOps 不是绕开 K8s 另起系统，而是长在托管底座上（3.1 主从：底座为主旨服务）。 **承上启下**：承第 3 章范式定纲（进入 How：底座施工；制品由第 2 章供给）；启第 5 章调谐闭环（控制器跑在底座上）。

> **边界声明**：本章只讲架构与治理；声明式调谐闭环归第 5 章，运行时深度机制不展开（V1.7 归 V2），调度细节归第 6 章，网络存储归第 7 章。超出部分归 V2。
