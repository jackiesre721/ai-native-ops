...
| Kubernetes 底座 | 第 4–7 章 | **L1 基座** | 资源调度、运行时、网络存储 |
| 声明式架构 | 第 5 / 8 章 | **L1 机械自治** | 期望状态 → 调谐 → 实际状态 |
| GitOps 交付 | 第 9 章 | L1 延伸 | 交付层声明式同步 |
| 灰度变更治理 | 第 10 章 | L1 → L2 过渡 | 变更风险护栏（观测决策雏形） |
| OTel 全域可观测 | 第 11 章 | L2 输入 | 全域信号采集（指标/日志/链路） |
| SRE 稳定性治理 | 第 12 / 13 章 | **L2 运维自治** | SLO / 错误预算 / 应急 / 成本 |
| 平台工程能力封装 | 第 14 章 | L2 支撑 | 能力黄金路径、屏蔽复杂度 |
| 运维自治 | 第 15 章 | **L2 核心** | 观测 → 处置 → 校验闭环 |
| LLM 建议式分诊 | 15.4⑤ | **L3 智能自治** | 建议式核心引擎（AI 驱动、人审核） |
| 评测门禁与落地路径 | 15.5 | **L3 核心** | 建议 → 采纳 → 执行 → 反馈闭环 |
...
全书 15 章核心正文 + 2 个附录，分五篇，不再新增章节（**V1.9 主旨聚焦**：第 17/18 章"AI 负载运维"移出 V1——全书主旨聚焦"用大模型与智能体重构运维"，L3 智能自治承载于 15.4⑤/15.5 Agent 引擎，第 15 章升格全书终章。**V1.11**：AI 训练/推理负载内容全书清除，不再以"范式认知/V2 预埋"保留（原第 6 章 Containerd 已于 V1.7 删除）。**V1.12**：编号重排连续——原第 7–16 章前移为第 6–15 章，文件名与交叉引用同步重写，章号 1–15 无空洞）。
...
- [第11章 OpenTelemetry全域可观测体系](11-opentelemetry-observability.md) — 技术栈永久锁死·数据底座支柱
...
每章文首均带：状态标记、篇归属、理论核心标记、技术栈锁死声明、边界声明，以及**承上启下链条句**（承上一章、启下一章，V1.10 起强制链相邻章）；每章文末带**「下一章预告」**（第 15 章为「全书收束」，六支柱终局对账）。

## 文档状态

- **状态**：正文已撰写完成并通过一轮自评，待终审定稿。
- **章节命名**：`NN-title.md`，`NN` 为章号（01–15，V1.12 起编号连续），按章号顺序阅读；附录为 `appendix-a.md` / `appendix-b.md`。
