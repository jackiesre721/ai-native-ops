...
> 本章定位：建立全书第一块地基——不可变基础设施与容器软件供应链。边界纯净，不涉及智能层内容（分诊器制品的供应链纪律复用本章，16.5）。
> **主线定位**：本章为制品不可变是声明式期望状态可信的前提——L1 机械自治的制品地基（三层自治总览见 1.5，理论核心为第 5/16 章——L3 智能自治承载于 16.4⑤/16.5 运维 Agent 引擎）。 **主旨绑定（V1.4）**：AI 原生运维的制品地基——分诊器的 prompt/评测集同样走这条不可变供应链（进 Git、走 PR，16.5），制品不可变是 Agent 引擎可回放运行的物理前提。 **承上启下**：承第 1 章演进判断（制品可信是一切的物理起点）；启第 3 章立论宪法——地基就绪即可定纲 AI 原生运维范式；镜像在此交付，底座自第 4 章起运行。

> **边界声明**：本章只讲容器镜像供应链（构建→仓库→追踪→扫描→发布），不展开深度容器运行时机制（V1.7 瘦身归 V2）。**2↔9 分工**：本章管"制品可信与不可变"（Artifact 层，拿什么东西运行）；"生产状态的声明式表达"（配置/策略/观测/交付的 Desired State）归第 9 章。超出部分归 V2。

> **云生态锚点（V1.1）**：镜像仓库 = **阿里云 ACR 企业版**主参考（AWS ECR 对照）；构建跑在 CI（主例 GitHub Actions，ACR 自动构建一句对照）；生产拉取免密走 ACK 免密组件 + RRSA/节点云身份（4.2 已落地）。自建 Harbor 不讲——同步、扫描、高可用全自建，仅此一句对照。
...
| 日常动作 | 可变运维（SSH 时代） | 不可变运维（本书） |
|---|---|---|
| **改配置** | 登机 vim + 重启进程，改完即漂移 | 改 Git values → ArgoCD 同步 → Pod 重建 |
| **扩容** | 新机手工装环境半天，装出"第 N 种状态" | ACK 节点池同一镜像开机即入池（7 章） |
| **回滚** | "上次改了什么"无人记得，靠回忆拼 | 回 Git 上一版本/上一 digest，重建即回 |
| **审计** | 逐个问登过机的人，按天计 | 问 Git 与 ACR，按分钟计 |

权衡的核心：**不可变牺牲"单次快速修改"，买回"规模化下的可预测性"**——100 个节点各改各的，复杂度指数爆炸；100 个节点状态完全一致，复杂度线性可控。
...
3. **状态外置**：有状态部分（数据）外置到 PV/对象存储/数据库，实例可随时销毁重建（8 章）。
...
```bash
# 改配置：改 Git → ArgoCD 同步 → Pod 重建（9/10 章），漂移在源头消失
git commit -am "tune: gateway timeout 3s -> 5s" && git push

# 回滚：回 Git/镜像上一版本（应急白名单内，13.3），重建即回
...
- 云服务映射：镜像不可变 = ACR digest/tag 锁定（2.4）；配置不可变 = Git 真相源 + Helm/ArgoCD（9/10 章）；节点不可变 = ACK/EKS 节点池托管替换。
...
- 镜像用 digest 锁定，`latest` 禁入生产（2.4 Do/Don't 表）。
- 配置全声明式，禁止 SSH 改生产（变更走 GitOps）。
- 节点随节点池自动升级/自动修复滚动重建，消除累积漂移（4.4）。
- 状态全部外置，实例无状态化。
...
```yaml
# charts/demo-api/values-prod.yaml——生产引用一律 digest 或不可变 tag（10 章承接）
image:
...
- 高危漏洞自动阻断发布，不依赖人工检查。
- 生产制品来源单一（ACR 企业版），禁止从公网随意拉取。
- 发布全走 GitOps，禁用 kubectl 直推生产。

### 自动化/自治闭环

受控供应链是第 10 章 GitOps 的前置：GitOps 的"唯一可信源"必须建立在"制品可信"之上，而制品可信由供应链保证。本节为 L1/L2 自治提供"变更可控"的制品基座——从代码到生产每步可追溯、可校验、可回滚。
...
USER app                           # 生产禁改：非 root 运行
ENTRYPOINT ["python", "-m", "src.app"]
# 大体积数据文件不进镜像：字体包/资源包等由部署侧 PVC 只读挂载注入（8.3 OSS 只读分发承接）
```

大文件进镜像的代价一句话：把 GB 级资源文件打进镜像层，镜像膨胀数倍，换资源=全节点全量重拉——**大文件是数据制品不是镜像层，OSS 外置 + 只读挂载是正解**（8.3）。体感：4GB 资源文件进镜像，等于每次发布让每个节点重下一次"整个网游客户端"；资源外置（OSS 注入）时，换资源只重拉几 MB 的代码层。
...
- 云服务映射：推送目标 = ACR 企业版（对照 ECR：registry 换 `<acct>.dkr.ecr.<region>.amazonaws.com` + aws-actions/amazon-ecr-login）。若无自建 CI，ACR 自动构建绑定代码源亦可触发构建（产品能力，字段以官方文档为准）；本书主例 GitHub Actions——扫描/签名闸门更好挂。
- 数字：~180MB 镜像 ACR 内网拉取 3–10 秒 vs 单阶段 ~1.2GB 的 30–90 秒；重型分层镜像 2–4GB 拉取 20–60 秒（大文件外置 OSS 是正解，8.3）。
...
- 基础镜像强制精简（distroless/alpine/slim），禁用重型基础。
- secrets 绝不入镜像层（构建期注入 + 运行时 ESO/KMS，附录 A）。
- 镜像体积与层数设上限，CI 自动校验（如 >500MB 通用服务告警）。
...
|---|---|---|---|
| 实例规格 | 基础/标准/高级独享实例 | 按仓库 + 计费存储 | 规格=同步并发与分发带宽，多团队标准版起 |
...
保留 30 tag 的存储账：通用服务 ~180MB × 30 ≈ 5.4GB/仓库，100 个服务 ≈ 540GB；重型镜像若 3GB × 30 tag = 90GB/仓库——又一个"大文件不进镜像"的理由（8.3 OSS 外置）。存储单价以官网当期价为准。体感：540GB 已装不下一块 512GB 的笔记本硬盘——100 个"小"服务静默吃掉一块盘。
...
```bash
# 签名：CI 内完成（2.3④），对 digest 签——tag 可能被覆盖，digest 永不
cosign sign --yes --key env://COSIGN_PRIVATE_KEY \
...
- 生产制品强制不可变（tag 不可变 + 删除保护）。
- 生产来源强制单一 + 准入校验（验签/来源白名单）。
- 安全拦截基线随威胁演进，每季度更新扫描策略。
