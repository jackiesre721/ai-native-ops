...
> 本章定位：建立全书第一块地基——不可变基础设施与容器软件供应链；制品不可变，声明式的期望状态才可信。
> **主线定位**：制品不可变是 L1 机械自治的地基，也是 Agent 引擎可回放运行的物理前提——分诊器的 prompt/评测集同样走这条供应链（进 Git、走 PR，15.5）。**承上启下**：承第 1 章演进判断（制品可信是一切的物理起点）；启第 3 章立论——地基就绪即可定纲 AI 原生运维范式；镜像在此交付，底座自第 4 章起运行。
