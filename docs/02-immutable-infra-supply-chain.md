# 第2章【Agent 制品地基】不可变基础设施与容器软件供应链治理
...
受控供应链是第 9 章 GitOps 的前置：GitOps 的"唯一可信源"必须建立在"制品可信"之上，而制品可信由供应链保证。本节为 L1/L2 自治提供"变更可控"的制品基座——从代码到生产每步可追溯、可校验、可回滚。分诊器的 prompt 与评测集走的也是这条链（15.5②：进 Git、走 PR、带版本上线）——"智能制品"不因是文本就豁免供应链纪律，引擎的可审计与可回放由此保证。
...
**供应链投毒三面与防御映射**（精简与锁定解决"面"，这一面解决"被谁喂"——构建链路的对抗性风险）：

| 攻击面 | 手法 | 防御（本书锚点） |
|---|---|---|
| **基础镜像投毒** | 上游基础镜像仓库被接管/新层植入恶意内容，`python:3.12` 这类浮动 tag 拉到什么全看运气 | digest 锁定（本节版本锁定）——基础层只认 `sha256:...`，升版本=换 digest 走完整评审（2.3① 注释） |
| **依赖混淆** | 公共源与私仓同名包，公共源版本号更高，构建机"就高不就低"拉了投毒包 | 私仓代理 + 显式 scope：`pip index-url`/`npm registry` 锁内网源、私有包名加 scope 前缀；CI 构建环境**无公网出口**，只有私仓一条取数路 |
| **仿名包** | `req uests` 类仿名/抢注包，等人手滑装上 | 依赖清单进 Git 走 PR 评审（8.3）；CI 生成 SBOM 进扫描门禁（2.2 ②）；运行镜像验签准入（2.3④，附录 A） |

三面防御的共同前提：**构建环境是生产系统，不是开发机**——CI 凭据最小化（只读源仓库 + 只写 ACR 一个 repo）、无交互 shell、每一步可追溯到 commit。这行字同时是 15.5②「智能制品走同一条供应链」的安全注脚：prompt 与评测集的仓库同样适用这三面。
...
精简安全镜像是供应链自动化的质量基线：**自动扫描（2.2）能高效拦截的前提，是攻击面已被多阶段构建收敛**——镜像精简，扫描才快、误报才少、门禁才可靠。本节为 L2 自治的"风险识别"提供低噪声输入。"先收敛、再设门禁"的思路与 15.5② 评测门禁同构——评测集干净（负例真实），80% 准确率门槛才有意义。
...
仓库治理是供应链自治的门禁层：**自动扫描（2.2）的拦截决策，要落到仓库的入库/上线策略上才有效**——不合规制品在仓库环节被拒绝，无需人工介入。本节为 L2 自治提供"自筛"执行点。入库策略"不合规即拒"与 15.4② 白名单"名单外即人工"是同一条治理哲学——每个自动动作背后，都要有一道制度化的拒绝面。
