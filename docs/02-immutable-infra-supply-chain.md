...
> **主线定位**：本章为制品不可变是声明式期望状态可信的前提——L1 机械自治的制品地基（三层自治见 1.5；L3 = 运维 Agent 引擎，15.4⑤/15.5）。 **主旨绑定（V1.4）**：AI 原生运维的制品地基——分诊器的 prompt/评测集同样走这条不可变供应链（进 Git、走 PR，15.5），制品不可变是 Agent 引擎可回放运行的物理前提。 **承上启下**：承第 1 章演进判断（制品可信是一切的物理起点）；启第 3 章立论宪法——地基就绪即可定纲 AI 原生运维范式；镜像在此交付，底座自第 4 章起运行。

> **边界声明**：本章只讲容器镜像供应链（构建→仓库→追踪→扫描→发布），不展开深度容器运行时机制（V1.7 瘦身归 V2）。**2 ↔ 8 分工**：本章管"制品可信与不可变"（Artifact 层，拿什么东西运行）；"生产状态的声明式表达"（配置/策略/观测/交付的 Desired State）归第 8 章。超出部分归 V2。
...
| **扩容** | 新机手工装环境半天，装出"第 N 种状态" | ACK 节点池同一镜像开机即入池（6 章） |
...
大文件进镜像的代价一句话：把 GB 级资源文件打进镜像层，镜像膨胀数倍，换资源=全节点全量重拉——**大文件是数据制品不是镜像层，OSS 外置 + 只读挂载是正解**（7.3）。体感：4GB 资源文件进镜像，等于每次发布让每个节点重下一次"整个网游客户端"；资源外置（OSS 注入）时，换资源只重拉几 MB 的代码层。
