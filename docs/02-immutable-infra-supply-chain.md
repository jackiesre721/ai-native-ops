...
**② AI 场景分层 Dockerfile**（vLLM 推理服务；变更频率决定层序，最常变的放最上层）：

```dockerfile
# syntax=docker/dockerfile:1.7
########## 层 1：vLLM 基础层——重而不常变：CUDA + PyTorch + vLLM，8–10GB，全公司共享
...
ENV PYTHONPATH=/workspace \
    HF_HOME=/tmp/hf                # 可调：模型/缓存路径指向只读挂载（17.3 的 PVC 注入）
RUN useradd --create-home --uid 1000 vllm && chown -R vllm /workspace
USER vllm                          # 生产禁改：非 root 运行
...
- 数字：~180MB 镜像 ACR 内网拉取 3–10 秒 vs 单阶段 ~1.2GB 的 30–90 秒；AI 镜像 8–10GB 拉取 1–3 分钟（模型入镜像则 10–35 分钟，17.3）。
