...
> **主线定位**：本章为L2 运维自治的人工侧——SLO/应急/复盘是自治的人工兜底与反哺源（三层自治总览见 1.5，理论核心为第 5/16 章——L3 智能自治承载于 16.4⑤/16.5 运维 Agent 引擎）。 **主旨绑定（V1.4）**：人机边界在此划定——16.5 评测门禁复用同一套 SLO 口径；自治的人工兜底、白名单外审批、复盘反哺都源于此章，AIOps 不豁免应急纪律。 **承上启下**：承第 12 章信号源；启第 14 章 SRE 稳定性与成本（口径既定，收束稳定性体系）——自治闭环在第 16 章接走这些口径。

> **边界声明**：本章不展开专属 OnCall 平台、不展开混沌工程平台、不展开 CIS 详细加固。以上归 V2。**SLO 职责锁**：本章是 SLO 的"人工应急"视角（SLO/错误预算/告警/故障/OnCall）；SLO 驱动的自动化处置归第 16 章。告警栈跑在锁死栈上（vmalert + Alertmanager，第 12 章），告警通道接云与 IM（电话/钉钉/飞书）。
...
治理目标数字：**全员日告警 ≤20 条；单人值班日 ≤10 条；P0 ≤1 条/周**。每条告警的 annotation 必须写"收到后做什么"——写不出处置动作的告警不该存在。

**智能告警的两层（V1.6）**：规则层（本节五环节：分级/收敛/降噪/静默/闭环）+ 建议层（LLM 语义分诊与处置建议，制品与护栏见 16.4⑤）——建议层给每条 P0/P1 附"根因假设 + 建议动作 + 置信度"，只进工单、不进执行通道；语义聚合（跨源告警自动归并）与自动 RCA 引擎归 V2（3.1 对照表）。

### 最小可行方案

1. **分级**：P0（全站故障）/P1（核心受损）/P2（部分受损）/P3（潜在风险），与 13.3 分级矩阵同一张表。
...
- name: node
  ...
  - alert: PodHighRestart
    ...
    annotations:
      ...
      action: "kubectl logs --previous 看崩溃原因；7.1 探针排障路径"
```
...
告警标准化是 L2 运维自治（16 章）的触发信号：自治的"风险识别"依赖高质量告警——噪音会触发误处置，漏报会错过处置。治理是自治可靠的前提。
...
**① SLI 口径与 recording rules**（vmalert 规则，指标来自第 12 章）：
...
预算消耗看板：Grafana 用 `1 - (实际可用性 / SLO)` 画剩余预算曲线，剩余 <25% 变红（12 章栈）。
...
SLO 是 L2 运维自治（16 章）的决策标尺：自治处置（扩缩/限流/回退）的目标是"守住 SLO"，burn-rate 是自治触发与发布冻结的依据。没有 SLO，自治没有目标。
...
P0–P3 分级矩阵（与 13.1 告警 severity 同一张表，全公司唯一）：
...
1. **发现**：告警驱动（13.1），先于用户感知。
2. **定位**：三支柱协同（12 章），按排查路径。
3. **止损**：优先恢复（回退/限流/扩容），不必等根因。
4. **恢复**：以 SLO 恢复为准（13.2），客观确认。
...
应急时直接改线上（rollback/scale）违反"Git 唯一真相源"（10 章）。处置规范：
...
LLM 分诊建议（16.4⑤）可作根因五问的第一稿草稿——复盘时把当时生成的 `cause_hypothesis` 与人工结论对照，差距本身就是分诊器的改进输入（建议采纳率度量的来源）。
...
# 验证：调用方超时配置（如 500ms）是否触发降级路径，观察 12 章链路追踪里的重试痕迹
...
**④ 与 SLO 体系联动**（混沌的进阶价值）：注入期间观察 13.2 的 burn-rate 告警是否在预期窗口触发——告警没触发比自愈失败更危险（真故障时会漏报）。这就是"验证自治与告警声称的能力"的闭环。

### 典型故障案例

团队"杀 Pod"注入发现某服务副本全扎堆同节点（无反亲和），杀一个节点全挂。补 topologySpreadConstraints（7 章）后再注入，单节点故障不再全挂。另一次注入暴露探针 initialDelaySeconds 过长——自愈耗时 210s，调参后 47s。
...
> **下一章预告**：应急与口径既定，稳定性体系收束——第 14 章讲 SRE 稳定性与资源成本治理：MTTR 趋势、复盘反哺与 FinOps 极简模型。
