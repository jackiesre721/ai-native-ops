# 第11章【L2 输入】OpenTelemetry全域可观测体系
<!-- 第四篇 可观测与稳定性 ｜ 常规章（可观测参考栈） ｜ 状态：终审中 -->

> 🧭 **本篇路标 · 第四篇「全域可观测与 SRE 稳定性体系」（第 11–13 章）**：本篇建立 L2 运维自治的输入与标尺——全域信号（OTel）、SLO 口径与错误预算、应急纪律、容量与成本护栏。15.5 上下文装配器的数据四件套全部由本篇供给；没有 SLO，Agent 处置就没有校验标尺；没有预算护栏，一次误信号就可能烧穿账单。L3 不是跳过 L2 的空中楼阁（1.5 不跳级）。

> 本章定位：第四篇开篇——建立全域可观测体系，是全书可观测技术栈的锚点，后续观测环节与 15.5 上下文装配器均复用此栈；「托管可观测 vs 自建」的取舍在 11.1 用决策表一次定调。
> **主线定位**：全域信号采集是 L2 运维自治的输入——15.5 上下文装配器的四件套（告警/变更/SLO/相似工单）全部由此栈供给；没有统一可观测，L3 智能自治就是瞎子（15.5①）。**承上启下**：承第 10 章灰度判断（观测校验的需求在此展开为全域体系）；启第 12 章告警（数据底座 → 智能告警的信号源）。
...
**决策维度再上移一层**：这张表只回答了「自建 vs 托管」一问，完整选型是三问——① **Build vs Buy**：可观测是 SRE 的手感所在、值得自建（本书立场）；非核心能力买托管不丢人。② **通用 vs 专用**：VM/Loki/Tempo 是通用件，专用 APM 换深度但吃定厂商锁定。③ **组织规模是最大决策变量**：<50 节点或没有 2 人以上养这套栈，托管省心是正确答案；数百节点量级，自建的分析自由度与成本优势才兑现——参考栈的意义是「原理与工具无关」，不是「自建是唯一正解」。
...
**演进方向（一句话定向，深度归 V2）**：无侵入采集的下一站是 **eBPF**——不改应用代码拿到内核级网络/系统调用/精确延迟数据，遗留系统与第三方黑盒场景的关键补充路径；本版以 OTel SDK + spanmetrics 兜底为正解。
