...
一张 A10 节点按量约 ¥5–10/时、一台 8 卡 A100 节点月账单十万级量级——最贵的资源进了集群却用不上：Pod 申请 `nvidia.com/gpu` 直接 Pending、调度器不知道哪些节点有卡、AI Pod 与普通 Pod 混部互相拖垮。**异构算力对默认 K8s 是"看不见、调不动、管不住"，最贵的资源反而最难用**。
...
|---|---|---|---|---|---|
| A10 | 24G | 推理主力（7B–14B） | gn7i 系列（如 ecs.gn7i-c32g1.8xlarge，1 卡） | g5（A10G 24G） | A10 单卡节点 ¥5–10/时；g5.xlarge ≈ $1/时 |
...
权衡的核心：**推理默认 A10 打底、训练/大 batch 才上 A100/H800**——A10 单位算力成本约为 A100 的 1/3，小模型 serving 不占大卡。GPU 池化深度方案（vGPU 平台、池化软件）归 V2，一句带过不进正文。
...
一张 A10 三方共用：白天推理、晚上训练、外加一个常驻小模型——一周之内两次互相 OOM；同时另一个 namespace 的批处理把整池 GPU 占满，推理排队。**GPU 不精细化管控，要么 OOM 互踩、要么整卡独占浪费**，¥5–10/时一张的卡利用率常年 <30%。
...
kubectl -n llm-prod get pods -o wide    # 3 个副本应落在同一张卡上（共享调度，而非各占一卡）
```

云服务映射：精细管控落在 **ACK cGPU 组件 + ECS GPU 节点**（对照 **EKS：GPU Operator device plugin + 时间分片，无显存隔离对应物**）；DCGM exporter 双云同一套（NVIDIA 开源）。数字：一张 A10 ¥5–10/时（以官网当期价为准），cGPU 把 3 个小模型塞进一张卡，单模型算力成本降到约 1/3。
...
- 盲目多副本：GPU 副本即卡（A10 ¥5–10/时/张），为"高可用"翻倍副本，成本直接翻倍。
...
- 节点故障：节点池自动修复 cordon→drain→替换（4.2），Pod 在新节点重拉——**单副本推理 MTTR ≈ 节点替换 + 模型加载 ≈ 5–8 分钟**（量级估算）；业务不可接受才加第二副本（+¥5–10/时/卡，以官网当期价为准）。
- 多副本时必配 PDB（`minAvailable: 1`）与反亲和（7 章）；单副本不配 PDB（防 drain 死锁）。

云服务映射：负载跑在 **ACK GPU 节点池**之上（对照 **EKS + g5/p4d 托管节点组**），服务暴露经 **SLB/ALB**（对照 ELB，8 章）——云平台负责节点替换，K8s 负责重拉，探针体系决定"何时接流"。数字：单副本 A10 包月 ≈ ¥3,000–4,500/月（较按量省 40%+，以官网当期价为准）；盲目 3 副本热备每月多烧数千到上万元，多数内部场景用不上。
