...
> **主线定位**：本章为定义 L3 的运维对象与难点——AI 篇总纲，三层自治主线在此完成对象扩展（三层自治总览见 1.5，理论核心为第 5/16/18 章）。
...
还有一组词面相近、必须钉死的区分——**AI 在运维体系中的双重视角**：AI 作为**负载**（被运维对象，特性与生命周期见 3.2/3.3 与 18.8），以及 AI 作为**运维主体**（运维 Agent——L2/L3 闭环中"推理分析"环节的载体）。全书 AI 章节按**负载视角**组织；**主体视角不另起体系**：运维 Agent 的工程形态就是既有闭环制品本身（决策规则表、白名单、护栏、影子模式，见 16.4/18.9），托管产品（阿里云 STAROps / AWS DevOps Agent，2026 年起相继商业化）是这套闭环的商业产品化。
...
先做一个思想实验（先自己猜答案，再往下读）：

> 墨丘里商城的 AI 平台团队宣布"统一技术栈"：训练任务、推理服务 reco-llm、客服 Agent，三类负载全用 Deployment + HPA、全落同一个节点池——一套模板、一套流程、一套值班，听起来是无懈可击的工程美德。**先猜：上线一周之内，最先出事的是哪一类负载？**

先别往下翻，认真猜十秒。一周战报揭晓——**三类全出事，只是死法不同**：训练任务最先倒下，一次例行滚动更新把跑了 20 小时的训练 Pod 当"旧副本"替换，进度归零，白烧一天 GPU——数百元卡时打水漂；客服 Agent 死在当晚流量回落时，HPA 缩容把正在对话的 Pod 回收，用户的话断在半句；reco-llm 死得最慢也最贵——CPU 信号对 GPU 推理失真、扩容迟钝，大促流量涌入时请求排队、首 Token 延迟飙升。甚至一次抢占式实例回收，还把误调度到同池的线上推理一起带走。**三类负载的运维特性与云上资源形态差异巨大，用同一种工作负载模型硬套，每一类都出问题**——统一是美德，统一错对象就是灾难。
...
- **权重即显存门槛**：FP16 下 7B 模型权重 ≈14 GB、32B ≈65 GB——给个体感：14 GB 约等于一个 300 万行代码大仓库的 100 倍体量，而这份"仓库"每次冷启动都要整体搬进显存；单卡 24 GB（A10 量级）放不下 32B 权重，必须多卡分片或量化；权重通常吃掉单卡显存的 50%–90%，剩下的才轮到 KV 缓存与激活。
- **加载是分钟级**：冷启动 = 从 OSS/ACR 拉取制品 + 权重装进显存，7B 量级约 2–5 分钟，比普通服务秒级拉起慢两个数量级——2–5 分钟的体感：用户等对话开始的时间，够起身泡一杯茶；推理扩容必须预热，不能等流量到了再扩。
...
> **Agent 边界预告**：Agent 服务与普通推理的运维差异（长生命周期、状态依赖、多工具调用、重试风暴）全书统一在**第 18.8 节**展开（边界锁死：不拓展 Agent 框架、RAG、MCP 等生态）。
...
| 难点 | 量级锚点 | 治理约束 | 代价 |
|---|---|---|---|
| 长任务 | 训练小时~天级；无 checkpoint 时一次中断 = 进度清零；抢占式回收预告仅约 5 分钟（EKS Spot 为 2 分钟）——从收到预警到实例消失，只够读完一条告警，手动救场的时间窗为零，续跑全靠 checkpoint（17 章） | 支持检查点 + 不可中断 + 优雅终止 | 工作负载复杂度上升 |
...
| 推理波动 | 同一服务 TTFT 可在 0.5 s 与 5 s 之间波动（10 倍）——0.5 s 是"秒回"的即时感，5 s 用户已开始怀疑卡死准备刷新，prefill 与 decode 必须分开度量（TTFT/TPOT，18 章） | 区分 prefill/decode 指标，分阶段治理 | 可观测语义更细 |
