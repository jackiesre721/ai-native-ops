...
> **主线定位**：本章为L2 运维自治的人工侧——SLO/应急/复盘是自治的人工兜底与反哺源（三层自治见 1.5；L3 = 运维 Agent 引擎，15.4⑤/15.5）。 **主旨绑定（V1.4）**：人机边界在此划定——15.5 评测门禁复用同一套 SLO 口径；自治的人工兜底、白名单外审批、复盘反哺都源于此章，AIOps 不豁免应急纪律。 **承上启下**：承第 11 章信号源；启第 13 章 SRE 稳定性与成本（口径既定，收束稳定性体系）——自治闭环在第 15 章接走这些口径。

> **边界声明**：本章不展开专属 OnCall 平台、不展开混沌工程平台、不展开 CIS 详细加固。以上归 V2。**SLO 职责锁**：本章是 SLO 的"人工应急"视角（SLO/错误预算/告警/故障/OnCall）；SLO 驱动的自动化处置归第 15 章。告警栈跑在锁死栈上（vmalert + Alertmanager，第 11 章），告警通道接云与 IM（电话/钉钉/飞书）。
...
**智能告警的两层（V1.6）**：规则层（本节五环节：分级/收敛/降噪/静默/闭环）+ 建议层（LLM 语义分诊与处置建议，制品与护栏见 15.4⑤）——建议层给每条 P0/P1 附"根因假设 + 建议动作 + 置信度"，只进工单、不进执行通道；语义聚合（跨源告警自动归并）与自动 RCA 引擎归 V2（3.1 对照表）。
...
云服务映射：电话通道用云语音（阿里云语音服务/钉钉电话；对照 AWS SNS→语音），告警系统本身用自建锁死栈（vmalert+Alertmanager，11 章）——规模 <50 节点时托管监控（ARMS/CloudWatch）也可承接，但为保持全书栈一致与成本可控，生产走自建。
