...
> **主线定位**：本章为L2 运维自治的人工侧——SLO/应急/复盘是自治的人工兜底与反哺源（三层自治见 1.5；L3 = 运维 Agent 引擎，15.4⑤/15.5）。 **主旨绑定**：人机边界在此划定——15.5 评测门禁复用同一套 SLO 口径；自治的人工兜底、白名单外审批、复盘反哺都源于此章，AIOps 不豁免应急纪律。 **承上启下**：承第 11 章信号源；启第 13 章 SRE 稳定性与成本（口径既定，收束稳定性体系）——自治闭环在第 15 章接走这些口径。
