# 第1章 云原生与AI原生运维架构演进
<!-- 第一篇 现代运维范式 ｜ 常规章（全书开篇·伏笔三层自治） ｜ 状态：正文写作中 -->

> 本章立论：聚焦架构痛点迭代（零基础科普），讲清运维三代范式跃迁，并在结尾伏笔全书三层自治模型，铺垫理论递进逻辑。

> **边界声明**：本章只做架构演进立论与全书模型伏笔，不展开具体技术实现；具体机制由后续各章承接。超出部分归 V2。
...
凌晨某支付网关 Pod 因内存泄漏 OOM 重启，值班接到告警，登录排查 12 分钟，`kubectl delete pod` 触发重建，故障总历时 18 分钟。两周后同一服务再次 OOM——因为重启只是治标，泄漏根因未修，**且重启动作完全依赖人**。

这是典型第一代范式。对照三代栈：有 `Deployment` + 自动重启 + 探针，稳态恢复是秒级；有可观测 + 自治，扩容/限流/告警自动化，甚至能在劣化前预防。同一类故障，在不同代际的运维体系里，代价差两个数量级。

### 根因定位

故障反复的根因不在内存泄漏本身（那是代码问题），而在**运维体系的控制模式**：稳态维持依赖人，没有期望状态的持续调谐，故障恢复是事件驱动而非持续收敛。每多一个服务，这类"依赖人"的恢复点就多一个，体系脆弱性随规模线性放大。这不是某次值班没做好，是体系结构决定的必然。
...
演进的终态是三层闭环叠加成型，这正是全书结构主线：

1. **机械自治**：期望状态 → 调谐循环 → 实际状态，秒级自愈。
2. **运维自治**：观测风险 → 决策处置 → 结果校验，分钟级治理。
3. **智能自治**：观测 → 推理 → 决策 → 执行 → 校验 → 反馈，动态自适应。
...
<!-- 全书三层自治模型伏笔小节，铺垫第5/16/18章理论递进逻辑 -->
