...
> **主线定位**：本章为L1→L2 过渡——变更风险护栏，观测决策的雏形（灰度=受控的状态迁移）（三层自治见 1.5；L3 = 运维 Agent 引擎，15.4⑤/15.5）。 **主旨绑定（V1.4）**：智能自治的变更护栏原型——Agent 引擎的 prompt/模型变更走评测门禁不直接上线（15.5② 护栏），其"受控迁移 + 观测 + 回退"的工程原型就是灰度。 **承上启下**：承第 9 章交付；启第 11 章可观测（灰度判断靠观测校验）。
