...
**时间线**（定级 P1：TTFT p99 3.2s 超 SLO(1s) 3 倍；可用性 99.5% 内部档 → 预算 216 分钟/30 天；容量受损 27 分钟、消耗 ≈2.4 分钟/月预算的 1.1%——27 分钟≈一整段午休；单副本 MTTR 5–8 分钟>5 分钟通知窗=排水来得及、补容量来不及）：
...
**时间线**（定级 P1；TTFT p99 <1s / TPOT p99 <100ms（18.6）；可用性 99.9% → 预算 43.2 分钟/30 天；影响 27 分钟、消耗 ≈0.6 分钟/月预算的 1.4%——这 27 分钟里用户眼看着 AI 摘要以慢十倍的打字机速度逐字蹦（TPOT 30ms→300ms））：
...
五个案例各聚焦一层；这里用一个假想场景把 **L1→L2→L3 在同一次故障中的接力**完整走一遍。设定：推理服务 order-llm（vLLM，A10 ×2，KEDA 2–8 副本）新版本存在显存缓慢泄漏（每分钟 +0.5%，低于灰度窗口的分析阈值所以 11.5 金丝雀未拦截）。时间线为设计目标值：

| 时刻 | 事件 | 自治层 | 动作与依据 |
|---|---|---|---|
| T+0 | 新版本全量完成，显存开始缓慢泄漏 | — | 11.5 灰度通过（分析窗口内未暴露慢性缺陷） |
| T+35min | 单 Pod `gpu_cache_usage_perc` 触顶，vLLM 抢占重算，TPOT 劣化 | **L1** | 探针健康、副本符合声明——机械自治"无事可做"：状态符合期望（5.3 自愈边界：配置错误不自愈，闭环反而会把现场拉回错误状态） |
| T+38min | TTFT p99 突破 SLO，burn-rate 慢烧（6×）双窗口命中，P2 告警 | **L2 输入** | 13.2 规则触发；16.4 处置契约：非快烧不自动处置 → 自动开工单——38 分钟才触发=慢性泄漏比任何人盯屏的耐心都长 |
| T+42min | 值班按 SOP 定位：`vllm:num_preemptions` 5m 增量 >0 → KV 段异常 | **L2 人工** | 13.3 止损优先：`argocd app rollback`（应急白名单，先停 auto-sync） |
| T+45min | 回滚完成，TPOT 恢复，显存释放 | **L1** | ArgoCD/Rollouts 收敛回旧版本（10.9 通道；目标 ≤5 分钟）——T+45min 全链恢复=一节站会的时间 |
| T+2h | 应急回写 Git + 复盘建档（根因五问） | **L2 制度** | 13.3 回写纪律；台账入 13.4，月度趋势纳入 14.4 |
| T+3d | L3 规则沉淀：显存斜率告警上线（利用率 15min 斜率 > 阈值 → 告警 + 自动降并发护栏，带人工阻断开关） | **L3** | 18.9 决策规则表新增一行；护栏 ConfigMap 默认"告警不自动处置"，观察一个周期后再开启自动 |
| 季度 | 混沌注入复演同类泄漏：L1/L2/L3 各环节按预期接力，全流程复验 | 全层 | 13.5 常态化风险验证 |

推演结论：**L1 不修"符合声明但行为错误"的故障——这正是它的边界而不是缺陷；L2 用 burn-rate 抓住人眼之外的慢性劣化并止损；L3 把这次人肉定位沉淀为永久规则，让同类故障下次在 T+15min 就被自动拦截**。三层不是三选一的方案，而是同一次故障里的三道接力防线：抽掉任何一层，T+45min 的恢复都会变成小时级——没有 L2 是拖到用户投诉，没有 L3 是每个季度重演一次，没有 L1 是每次 Pod 抖动都要人到场。
