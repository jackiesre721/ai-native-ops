...
> 案例锁死 3 个（V1.9：AI 负载两案随第 17/18 章移出；V1.11：AI 负载内容不再回归）——覆盖 L1 机械自治失效、L2 告警/SLO、云集成；末项为三案例×治理域汇总与全链路落地复盘，串联全书完整运维控制闭环。仅复用正文知识点落地验证，无任何新技术、新架构引入。每个案例统一复盘格式：**背景 → 时间线（HH:MM 精确到分钟）→ 排障实录（实际命令）→ 根因（含 13.3 根因五问中最重的两问）→ 处置与止损 → 长效整改（锚回正文章节）**；复盘五字段与根因五问的完整定义见 13.3，此处只引用不重复。案例环境均为 ACK 生产集群（对照 EKS），工具只用正文锁死栈：kubectl、aliyun CLI、ArgoCD/Rollouts、KEDA、vmalert/Alertmanager。 **主旨绑定（V1.4）**：主旨的实战验收——故障闭环验证"云原生底座 + Agent 引擎主旨"的全书架构。 **承上启下**：全书主线的实战验收（三案例闭环，复用正文，无新知识）。

---

## B.1 案例一：节点压力驱逐撞上慢自愈参数——L1 闭环在干活，但每次要 210 秒
<!-- 覆盖：L1 机械自治失效 ｜ 锚点：5.3 / 7.1 / 7.4 / 4.2 / 13.5 -->
...
**根因**：把节点推向临界的是 noisy neighbor 未设 limit（7.4），把"一次驱逐"放大成"23 分钟 P1"的是自愈参数从未实测——公网镜像、readiness 硬等、无 PDB 三项旧债叠加，L1 闭环"活着但很慢"（7.1 自愈时长预算表）。五问取最重两问：**为什么会坏**——资源治理缺位，节点长期高水位无预警；**为什么恢复用了这么久**——自愈时长从未按 13.5 实测，副本折半时 210s 的自愈等价于不可用。

**处置与止损**：扩容 4→8 副本到健康节点（白名单内，2h 回写 Git）；高水位节点排空后交节点池自动修复替换（4.2）。

**长效整改**（锚回正文）：

- 全容器 request/limit + LimitRange 兜底（7.4），节点内存水位告警线先于驱逐线。
...
|---|---|---|---|
| B.1 驱逐×慢自愈 | L1 机械自治 | 自愈参数从未实测；limit/PDB 缺位 | 5.3、7.1、7.4、4.2、13.5 |
...
全链路收束：三个案例的止损动作无一例外回到同两件事——**声明式真相源**（Git 期望状态 + 控制器/CCM/KEDA 的调谐闭环负责收敛）与 **SLO 口径**（预算消耗决定止损节奏与发布档位）。这正是全书核心业务闭环链路（不可变基础设施→K8s 底座→声明式→GitOps→灰度→OTel 可观测→SRE 治理→平台封装→运维自治→建议式引擎→评测门禁→AI 智能自治）在故障场景下的重演：B.1/B.3 验证 L1 机械自治（第 5 章）的可靠性与边界，B.2 验证 L2 运维自治（第 16 章）的触发信号与处置质量，B.3 同时验证云集成治理（4.2）——三层递进的故障面全部有着落。每个案例复盘出的根因模式（自愈未实测、告警未治理、云资源无台账）最终都反哺为正文对应章节的准入项、告警规则与自治输入——三层递进自治模型由此在一家企业完整跑通：它是可落地的工程体系，而非理论。

### 全链路推演：一次故障中三层自治的接力（假想场景）

三个案例各聚焦一层；这里用一个假想场景把 **L1→L2→L3 在同一次故障中的接力**完整走一遍。设定：订单服务 order-api 新版本存在数据库连接池缓慢泄漏（每分钟漏 2 个连接，低于灰度窗口的分析阈值所以 11.5 金丝雀未拦截）。时间线为设计目标值：

| 时刻 | 事件 | 自治层 | 动作与依据 |
|---|---|---|---|
| T+0 | 新版本全量完成，连接池占用开始缓慢爬升 | — | 11.5 灰度通过（分析窗口内未暴露慢性缺陷） |
| T+35min | 单 Pod 连接池触顶，请求排队，p99 开始爬升 | **L1** | 探针健康、副本符合声明——机械自治"无事可做"：状态符合期望（5.3 自愈边界：配置错误不自愈，闭环反而会把现场拉回错误状态） |
| T+38min | p99 突破 SLO，burn-rate 慢烧（6×）双窗口命中，P2 告警 | **L2 输入** | 13.2 规则触发；16.4 处置契约：非快烧不自动处置 → 自动开工单——38 分钟才触发=慢性泄漏比任何人盯屏的耐心都长 |
| T+39min | 分诊引擎 60s 内附建议：根因假设=连接池泄漏（证据：连接数线性爬升曲线与 14:20 发布时间吻合、近 2h 无其他变更）+ 建议动作=回滚最近发布（白名单 #2）+ 置信度高 | **L3 建议** | 16.4⑤ 建议式分诊——AI 驱动出建议，人审核做决策 |
| T+42min | 值班审核证据链后按 SOP 执行：回滚最近发布 | **L2 人工** | 13.3 止损优先：`argocd app rollback`（应急白名单，先停 auto-sync） |
| T+45min | 回滚完成，连接释放，p99 恢复 | **L1** | ArgoCD/Rollouts 收敛回旧版本（10.9 通道；目标 ≤5 分钟）——T+45min 全链恢复=一节站会的时间 |
| T+2h | 应急回写 Git + 复盘建档（根因五问）；建议采纳结果落台账 | **L2 制度** | 13.3 回写纪律；台账入 13.4，采纳字段回填 16.5③，月度趋势纳入 14.4 |
| T+3d | L3 规则沉淀：同类"连接数线性爬升 + 近期变更"告警模式经评测集回归后上线（护栏默认"告警不自动处置"，观察一个周期再开自动） | **L3** | 16.5② 评测门禁 + 16.4② 白名单 |
| 季度 | 混沌注入复演同类泄漏：L1/L2/L3 各环节按预期接力，全流程复验 | 全层 | 13.5 常态化风险验证 |
